...
void TexturedTerrainApplication::Render()
{
    ...
    DrawObject(m_terrainPatch, *m_terrainMaterial2,
    glm::translate(glm::vec3(0.0f, 0.0f, -10.0f)) * glm::scale(glm::vec3(10.0f)));

    DrawObject(m_terrainPatch, *m_terrainMaterial3,
    glm::translate(glm::vec3(-10.0f, 0.0f, 0.0f)) * glm::scale(glm::vec3(10.0f)));

    DrawObject(m_terrainPatch, *m_terrainMaterial4,
    glm::translate(glm::vec3(-10.0f, 0.0f, -10.0f)) * glm::scale(glm::vec3(10.0f)));

    // Water patches
    // (todo) 04.5: Add water planes
    float waterLevel = -3.0f;
    DrawObject(m_terrainPatch, *m_waterMaterial,
    glm::translate(glm::vec3(0.0f, waterLevel, 0.0f)) * glm::scale(glm::vec3(10.0f)));

    DrawObject(m_terrainPatch, *m_waterMaterial,
    glm::translate(glm::vec3(0.0f, waterLevel, -10.0f)) * glm::scale(glm::vec3(10.0f)));

    DrawObject(m_terrainPatch, *m_waterMaterial,
    glm::translate(glm::vec3(-10.0f, waterLevel, 0.0f)) * glm::scale(glm::vec3(10.0f)));

    DrawObject(m_terrainPatch, *m_waterMaterial,
    glm::translate(glm::vec3(-10.0f, waterLevel, -10.0f)) * glm::scale(glm::vec3(10.0f)));
}

void TexturedTerrainApplication::InitializeTextures()
{
    m_defaultTexture = CreateDefaultTexture();
    m_heightMapTexture1 = CreateHeightMap(m_gridX, m_gridY, glm::ivec2(0, 0));
...
void TexturedTerrainApplication::InitializeMaterials()
{
    ...
    m_terrainMaterial1->SetUniformValue("Heightmap", m_heightMapTexture1);
    ...
    m_terrainMaterial1->SetUniformValue("ColorTextureScale", glm::vec2(0.1f, 0.1f));
    m_terrainMaterial1->SetUniformValue("Range1", glm::vec2(-0.35f, -0.15f)); // dirt -> grass
    m_terrainMaterial1->SetUniformValue("Range2", glm::vec2(-0.05f,  0.20f)); // grass -> rock
    m_terrainMaterial1->SetUniformValue("Range3", glm::vec2( 0.55f,  0.65f)); // rock -> snow
    ...
    m_waterMaterial->SetUniformValue("ColorTextureScale", glm::vec2(0.1f, 0.1f));
...
std::shared_ptr<Texture2DObject> TexturedTerrainApplication::CreateHeightMap(unsigned int width, unsigned int height, glm::ivec2 coords)
{
    ...
    for (unsigned int j = 0; j < height; ++j)
    {
        for (unsigned int i = 0; i < width; ++i)
        {
            ...
            float gain = 0.5f;
            int octaves = 6;
            float x = (static_cast<float>(i) / static_cast<float>(width - 1)) + static_cast<float>(coords.x);
            float y = (static_cast<float>(j) / static_cast<float>(height - 1)) + static_cast<float>(coords.y);

            float coolPixel = stb_perlin_fbm_noise3(x, y, 0.0f, lacunarity, gain, octaves);
